...
ويُطبَّق FIND/REPLACE بالسكربت المحروس المعرَّف في `§28`: نص FIND هو مرساته، وقواعد FIND أدناه كلها شروط عليها.
...
- اقرأ عقد التسليم النشط وبيانات المرحلة قبل فحص أي ملف، من المنطقة النشطة في رأس الدفتر بأمر `awk '/^<!-- حدّ الأرشيف/{exit} {print}' docs/ai/ARCHITECT_EVIDENCE_LEDGER.md | head -n 120`، ويُمنع قراءة الدفتر كاملًا. والحدّ 120 سقف أمان لا حدّ قراءة.
- تحقق من المخرَج قبل الاعتماد عليه بعدّ عنوان `### عقد التسليم النشط` في بداية السطر: `awk '/^<!-- حدّ الأرشيف/{exit} {print}' docs/ai/ARCHITECT_EVIDENCE_LEDGER.md | grep -c '^### عقد التسليم النشط'`، فإن لم يعطِ 1 بالضبط، أو بلغ المخرَج 120 سطرًا، فالمخرَج مبتور أو منفلت — توقف وأبلغ المالك، ولا توصف الحالة بأنها مقروءة.
- العقد النشط هو أعلى عقد تسليم في المنطقة النشطة وحده، وما دونه سجل منسوخ لا يُعتد به حالةً راهنة.
...
توقف قبل أي تعديل عند: HEAD غير المتوقع، أو شجرة غير نظيفة، أو غموض يمنع تعديلًا آمنًا، أو حاجة إلى ملف خارج النطاق، أو اكتشاف أن المرحلة السابقة غير مغلقة. ويُستثنى من «HEAD غير المتوقع» وحده ما تنطبق عليه شروط `Expected Self-Reference Drift` المعرَّفة في `§21` من بروتوكول المعماري، فيُسجَّل بهذا الاسم ويُمضى فيه بإقرار المالك. ولا تُعلن «شجرة غير نظيفة» قبل استبعاد `Stale Index False Positive` المعرَّف في قسم التوقف من `§21` من بروتوكول المعماري، بـ`git update-index --refresh` ثم إعادة `git status --short`، ثم مقارنة `git hash-object <path>` بـ`git rev-parse HEAD:<path>` عند بقاء الفارق.

### عند اكتمال التنفيذ

- حدّث دفتر الأدلة ضمن حزمة التعديل نفسها، لا في خطوة لاحقة، بإدراج القيد في المنطقة النشطة تحت فقرات السلطة مباشرة وفوق أحدث عقد قائم.
...
- يلتزم تقرير الإكمال بأقسام هذا البروتوكول، ويضيف عقد تسليم مختصرًا لا يتجاوز 40 سطرًا بخطوة تالية واحدة. وهذا الحد انضباط صياغة لا ضامن قراءة.
...
- إذا خالف HEAD ما هو مقيَّد، ففُزْ أولًا بين انحراف حقيقي وانحراف `Expected Self-Reference Drift` المعرَّف في `§21` من بروتوكول المعماري، بأمرَي `git log --oneline <baseline>..HEAD` و`git diff --stat <baseline>..HEAD` ثم مقارنة بصمات ملفات النطاق. وبغير اجتماع شروطه يسري التوقف.
- سجّل بصمات ملفات النطاق من كائنات Git بأمر `git rev-parse HEAD:<path>` إلى جانب أي بصمة حية.
...
## 28. التطبيق المحروس وأسلوب الجلسة المستقلة

هذا القسم يوثّق الطريقة المجرَّبة حين لا يملك النموذج طرفية: النموذج يكتب الأوامر، والمالك ينفّذها في طرفيته (WSL) ويلصق مخرجها حرفيًّا. وهو لا يلغي قواعد FIND في `§9`، بل ينفّذها آليًّا فيمنع أخطاء اللصق اليدوي.

### التطبيق المحروس

- يُسلَّم كل FIND/REPLACE سكربتَ Python بالمكتبة القياسية داخل الأمر نفسه، يقرأ الملف بايتات ويحسب الناتج كله في الذاكرة قبل أي كتابة.
- المرساة هي نص FIND حرفيًّا. يعدّ السكربت ظهورها، فإن لم تظهر مرة واحدة بالضبط خرج برمز غير صفري ولم يكتب بايتًا.
- تُحفظ نهايات الأسطر المقيسة في الملف المستهدف نفسه ولا تُعمَّم من ملف إلى غيره: ملف CRLF يُقسَّم على `\r\n` ويُحفظ بـ`'\r\n'.join(lines) + '\r\n'`.
- يعلن النموذج قبل التنفيذ عدد الأسطر الناتج، ويقارنه السكربت بالمحسوب قبل الكتابة، وإذا مسّ التعديل أكثر من ملف فلا يُكتب أيٌّ منها حتى تنجح حراسات الكل.
- الملف الجديد يُكتب كاملًا من السكربت نفسه، بلا `...` ولا placeholder، وبنهايات الأسطر الغالبة في جيرانه المقيسة.
- لا يُطلب من المالك تحرير ملف بيده، ولا لصق نص في محرر، ولا لصق محتوى SVG.

### أسلوب الجلسة

- أمر واحد مجمَّع داخل `( … )` يقيس كل ما تحتاجه الخطوة، حتى لا يُغلق `exit` طرفية المالك، ويكون أمر قراءة لا يكتب ما لم يُعلن غير ذلك.
- يُعلن جدول التوقعات الرقمية قبل التنفيذ ثم يُقابَل بالمخرج. وإذا خالفه المخرج فالخطأ في التوقع حتى يثبت العكس، ويُعترف به مرة واحدة ثم تُصلح المادة.
- البرهان مطابقة بايتات وإعادة توليد وتشغيل فعلي، لا وصف.
- كل كتابة على فرع أُنشئ قبلها، وstage بمسارات صريحة، ولا `git add -A` ولا `git add .`، وفتح طلب الدمج ودمجه للمالك.
- حين يُسأل المالك يُسأل سؤالًا واحدًا محدَّدًا يُجاب بنعم أو لا، بلغة بسيطة.
- يُحدَّث الدفتر في طلب الدمج نفسه الذي يغيّر الحالة، فلا يحمل Handoff خارج المستودع حقيقة ليست فيه.
